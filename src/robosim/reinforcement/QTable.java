...
public class QTable {
    ...
    public int leastVisitedAction(int state) {
        ...
        int action = 0;
        for (int i = 0; i < q[state].length; i++) {
            if (visits[state][i] < small) {
                action = i;
                small = visits[state][i];
            }
        }
        return action;
    }
    ...
    public int senseActLearn(int newState, double reward) {
        double update = (1 - getLearningRate(lastState, lastAction)) * getQ(lastState, lastAction)
                + getLearningRate(lastState, lastAction) * (discount * getQ(newState, getBestAction(newState)) + reward);
